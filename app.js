...
/* Enterprise 4.0 RC1 Official Forms Engine */
let appLang=localStorage.getItem("uds_lang")||"en",activeSignatureField=null,signatureDrawing=false,signatureCtx=null;
function setLanguage(lang){appLang=lang==="mn"?"mn":"en";localStorage.setItem("uds_lang",appLang);applyLanguage();if(typeof renderSelectedForm==="function")renderSelectedForm();}
function applyLanguage(){let s=document.getElementById("languageSelect");if(s)s.value=appLang;document.querySelectorAll(".top-nav button").forEach(b=>{let id=b.dataset.tab;if(id==="documents")b.textContent=appLang==="mn"?"Баримт":"Docs";});}
function initSignaturePad(){let c=document.getElementById("signatureCanvas");if(!c||c.dataset.ready)return;c.dataset.ready="1";c.addEventListener("mousedown",sigStart);c.addEventListener("mousemove",sigMove);window.addEventListener("mouseup",sigEnd);c.addEventListener("touchstart",sigStart,{passive:false});c.addEventListener("touchmove",sigMove,{passive:false});window.addEventListener("touchend",sigEnd)}
function openSignaturePad(id,title){activeSignatureField=id;let m=document.getElementById("signatureModal"),c=document.getElementById("signatureCanvas"),h=document.getElementById("signatureTitle");if(!m||!c)return alert("Signature pad not available.");if(h)h.textContent=title||"Signature";m.classList.remove("hidden");signatureCtx=c.getContext("2d");signatureCtx.fillStyle="white";signatureCtx.fillRect(0,0,c.width,c.height);signatureCtx.strokeStyle="#111";signatureCtx.lineWidth=4;signatureCtx.lineCap="round";let ex=document.getElementById(id)?.value;if(ex){let img=new Image();img.onload=()=>signatureCtx.drawImage(img,0,0,c.width,c.height);img.src=ex}}
function closeSignaturePad(){document.getElementById("signatureModal")?.classList.add("hidden");activeSignatureField=null}
function clearSignaturePad(){let c=document.getElementById("signatureCanvas");if(c&&signatureCtx){signatureCtx.fillStyle="white";signatureCtx.fillRect(0,0,c.width,c.height)}}
function sigPos(e){let c=document.getElementById("signatureCanvas"),r=c.getBoundingClientRect(),p=e.touches&&e.touches[0]?e.touches[0]:e;return{x:(p.clientX-r.left)*(c.width/r.width),y:(p.clientY-r.top)*(c.height/r.height)}}
function sigStart(e){e.preventDefault();signatureDrawing=true;let p=sigPos(e);signatureCtx.beginPath();signatureCtx.moveTo(p.x,p.y)}
function sigMove(e){if(!signatureDrawing)return;e.preventDefault();let p=sigPos(e);signatureCtx.lineTo(p.x,p.y);signatureCtx.stroke()}
function sigEnd(){signatureDrawing=false}
function saveSignaturePad(){let c=document.getElementById("signatureCanvas");if(!c||!activeSignatureField)return;let data=c.toDataURL("image/png"),h=document.getElementById(activeSignatureField),p=document.getElementById(activeSignatureField+"_preview");if(h)h.value=data;if(p)p.innerHTML=`<img src="${data}">`;closeSignaturePad()}
function sigField(label,id){let n=appLang==="mn"?"Гарын үсэг оруулаагүй":"No signature captured";return `<div class="signature-box"><label>${esc(label)}</label><input type="hidden" id="${id}"><div class="signature-preview" id="${id}_preview">${n}</div><div class="signature-btn-row"><button type="button" onclick="openSignaturePad('${id}','${escAttr(label)}')">${appLang==="mn"?"Гарын үсэг":"Sign"}</button><button type="button" class="secondary" onclick="document.getElementById('${id}').value='';document.getElementById('${id}_preview').innerHTML='${n}'">${appLang==="mn"?"Арилгах":"Clear"}</button></div></div>`}
function docPrefix(type){return type==="pjo"?"PJO":type==="cascaded"?"COACH":"FORM"}
function generateDocNumber(type){let y=new Date().getFullYear(),p=docPrefix(type),n=savedForms.filter(f=>(f.doc_number||"").startsWith(p+"-"+y)).length+1;return `${p}-${y}-${String(n).padStart(6,"0")}`}
function saveFormsLocal(){localStorage.setItem("uds_enterprise_forms",JSON.stringify(savedForms))}
function getStatusClass(s){return "doc-status-"+String(s||"Draft").toLowerCase()}
function collectCurrentForm(){let data={};document.querySelectorAll("#currentFormPrintable input,#currentFormPrintable textarea,#currentFormPrintable select").forEach(el=>{if(el.type==="checkbox")data[el.id]=el.checked;else if(el.type==="radio"){if(el.checked)data[el.name]=el.value}else if(el.type!=="file")data[el.id]=el.value});return data}
function check(id,label){return `<label class="form-check"><input type="checkbox" id="${id}"> ${esc(label)}</label>`}
function yn(name){return `<div class="yes-no"><label class="form-check"><input type="radio" name="${name}" value="Yes"> Тийм / Yes</label><label class="form-check"><input type="radio" name="${name}" value="No"> Үгүй / No</label></div>`}
function q(num,text,id,ynb=true){return `<div class="form-question"><div class="form-question-title">${num}. ${esc(text)}</div>${ynb?yn("pjo_"+id):""}<textarea id="pjo_${id}_comment" placeholder="Тайлбар / Comments"></textarea></div>`}
function renderSelectedForm(){let t=document.getElementById("formType")?.value||"pjo";if(t==="pjo")renderPJOForm();else if(t==="cascaded")renderSCCForm();else renderPlaceholderForm(document.getElementById("formType")?.selectedOptions[0]?.textContent||"Form")}
function renderPJOForm(){let today=new Date().toISOString().split("T")[0];dynamicFormArea.innerHTML=`<div class="form-template" id="currentFormPrintable"><div class="form-header-dayan"><div class="form-logo"><span>D</span>AYAN</div><div class="form-title"><h2>АЖЛЫН ТӨЛӨВЛӨГӨӨТ АЖИГЛАЛТ</h2><p>PLANNED JOB OBSERVATION</p></div></div><div class="form-section"><h3>Ерөнхий мэдээлэл / Header</h3><div class="form-grid"><label>Ажлын байр / Workplace<input id="pjo_workplace"></label><label>Date<input id="pjo_date" type="date" value="${today}"></label><label>Job/task observed<input id="pjo_task"></label><label>Procedure name and #<input id="pjo_procedure"></label><label>Observer<input id="pjo_observer"></label><label>Supervisor<input id="pjo_supervisor"></label></div></div><div class="form-section"><h3>Reason for observation</h3><div class="form-check-grid">${check("pjo_reason_procedure","Job Procedure / Practice Update")}${check("pjo_reason_injury","Recent injury associated with job/task")}${check("pjo_reason_training","Training follow-up")}${check("pjo_reason_experienced","Experienced worker check")}${check("pjo_reason_other","Other")}</div><label>Other<input id="pjo_reason_other_text"></label></div><div class="form-section"><h3>Worker(s)</h3><div class="form-grid"><label>Worker #1<input id="pjo_emp1_name"></label><label>Occupation<input id="pjo_emp1_occupation"></label><label>Experience<input id="pjo_emp1_experience"></label><label>Worker #2<input id="pjo_emp2_name"></label><label>Occupation<input id="pjo_emp2_occupation"></label><label>Experience<input id="pjo_emp2_experience"></label></div><div class="form-question"><b>Is worker trained and licensed?</b>${yn("pjo_trained")}</div><div class="form-question"><b>Training records verified?</b>${yn("pjo_training_records")}</div></div><div class="form-section"><h3>Observations</h3>${q(1,"Could any practices or conditions observed result in personal injury or near miss?","q1")}${q(2,"Could any practices or conditions observed result in property damage?","q2")}${q(3,"Were procedures/standards followed?","q3")}${q(4,"What key points were noticed?","q4",false)}${q(5,"Was positive recognition given?","q5")}${q(6,"Was corrective information or instruction given?","q6")}${q(7,"Review for efficiency and production capability?","q7")}${q(8,"What should we consider changing in the interest of safety?","q8")}${q(9,"Additional comments/observations","q9",false)}${q(10,"Do procedures need to be revised?","q10")}</div><div class="form-section"><h3>Follow-up</h3><div class="form-grid"><label>Duration<input id="pjo_duration"></label><label>Start<input id="pjo_start_time" type="time"></label><label>Stop<input id="pjo_stop_time" type="time"></label><label>Responsible person<input id="pjo_responsible"></label><label>Due date<input id="pjo_due_date" type="date"></label></div><label>Recommended follow-up<textarea id="pjo_followup"></textarea></label></div><div class="form-section"><h3>Signatures</h3><div class="form-grid">${sigField("Worker #1 Signature","pjo_sig_emp1")}${sigField("Worker #2 Signature","pjo_sig_emp2")}${sigField("Observer / Crew Trainer Signature","pjo_sig_observer")}${sigField("Supervisor Signature","pjo_sig_supervisor")}${sigField("Superintendent / Area Manager Signature","pjo_sig_manager")}${sigField("Safety Coordinator Signature","pjo_sig_safety")}</div></div><button class="save-btn" onclick="saveFillableForm()">Save PJO</button><button class="secondary" onclick="printCurrentForm()">Official Print / PDF</button></div>`}
function renderSCCForm(){let today=new Date().toISOString().split("T")[0];dynamicFormArea.innerHTML=`<div class="form-template" id="currentFormPrintable"><div class="form-header-dayan"><div class="form-logo"><span>D</span>AYAN</div><div class="form-title"><h2>АЮУЛГҮЙ АЖИЛЛАГААНЫ ШАТАЛСАН КӨҮЧИНГ</h2><p>SAFETY CASCADING COACHING</p></div></div><div class="form-section"><div class="form-grid"><label>Date<input id="scc_date" type="date" value="${today}"></label><label>Learner<input id="scc_learner"></label><label>Coach #1<input id="scc_coach1"></label><label>Coach #2<input id="scc_coach2"></label><label>Department<input id="scc_department"></label><label>Team Name<input id="scc_team"></label><label>Job title<input id="scc_job_title"></label></div></div><div class="form-section"><h3>Safety process</h3><div class="form-check-grid">${check("scc_process_preshift","Pre-Shift Meeting")}${check("scc_process_discussion","Safety Discussion")}${check("scc_process_crm","CRM")}${check("scc_process_ptha","Pre-Task Hazard Assessment")}${check("scc_process_other","Other")}</div><label>Other<input id="scc_process_other_text"></label></div><div class="form-section"><label>Supervisor: what went well?<textarea id="scc_self_went_well"></textarea></label><label>Supervisor self-analysis / improvements<textarea id="scc_self_improvements"></textarea></label><label>1 Coaching Discussion<textarea id="scc_discussion1"></textarea></label><label>Coach suggested improvements<textarea id="scc_discussion1_improve"></textarea></label><label>2 Coaching Discussion<textarea id="scc_discussion2"></textarea></label><label>Summarize Coaching Session<textarea id="scc_summary"></textarea></label><label>Key Improvement Actions<textarea id="scc_actions"></textarea></label></div><div class="form-section"><h3>Signatures</h3><div class="form-grid">${sigField("Learner Signature","scc_sig_learner")}${sigField("Coach #1 Signature","scc_sig_coach1")}${sigField("Coach #2 Signature","scc_sig_coach2")}</div></div><button class="save-btn" onclick="saveFillableForm()">Save Safety Cascading Coaching</button><button class="secondary" onclick="printCurrentForm()">Official Print / PDF</button></div>`}
function saveFillableForm(){let type=document.getElementById("formType")?.value||"pjo",title=document.getElementById("formType")?.selectedOptions[0]?.textContent||"Form";let doc={id:"form_"+Date.now(),doc_number:generateDocNumber(type),type,title,status:"Draft",data:collectCurrentForm(),created_at:new Date().toISOString(),created_by:currentUser?.email||""};savedForms.unshift(doc);saveFormsLocal();renderDocumentCentre();alert(`${doc.doc_number}\n${title}\nSaved locally.`)}
function getVisibleDocuments(){return savedForms||[]}
function renderDocumentCentre(){let el=document.getElementById("documentCentreList");if(!el)return;let docs=getVisibleDocuments();el.innerHTML=docs.length?docs.map(f=>`<div class="document-card"><h3>${esc(f.doc_number||"UNNUMBERED")} — ${esc(f.title||"Form")}</h3><div class="doc-meta"><span class="doc-pill ${getStatusClass(f.status)}">${esc(f.status||"Draft")}</span><span class="doc-pill">${esc(f.type||"")}</span><span class="doc-pill">${new Date(f.created_at).toLocaleString()}</span></div><div class="doc-actions"><button onclick="viewSavedForm('${f.id}')">View</button><button onclick="editSavedForm('${f.id}')">Edit</button><button onclick="printSingleSavedForm('${f.id}')">Print</button><button onclick="updateDocumentStatus('${f.id}','Submitted')">Submit</button><button onclick="updateDocumentStatus('${f.id}','Approved')">Approve</button><button class="danger" onclick="deleteSavedForm('${f.id}')">Delete</button></div></div>`).join(""):"<div class='notice'>No documents yet.</div>"}
function updateDocumentStatus(id,status){let f=savedForms.find(x=>x.id===id);if(!f)return;f.status=status;f.status_updated_at=new Date().toISOString();f.status_updated_by=currentUser?.email||"";saveFormsLocal();renderDocumentCentre()}
function renderSavedForms(){let el=document.getElementById("savedFormsList");if(el)el.innerHTML="<div class='notice'>Use Document Centre for official records.</div>";renderDocumentCentre()}
function deleteSavedForm(id){if(!confirm("Delete this document from this device?"))return;savedForms=savedForms.filter(x=>x.id!==id);saveFormsLocal();renderDocumentCentre()}
function editSavedForm(id){let f=savedForms.find(x=>x.id===id);if(!f)return;showTab("forms");document.getElementById("formType").value=f.type||"pjo";renderSelectedForm();setTimeout(()=>Object.entries(f.data||{}).forEach(([k,v])=>{let e=document.getElementById(k);if(e){if(e.type==="checkbox")e.checked=!!v;else e.value=v;if(k.includes("_sig_")&&v){let p=document.getElementById(k+"_preview");if(p)p.innerHTML=`<img src="${v}">`}}}),50)}
function viewSavedForm(id){printSingleSavedForm(id,false)}
function printDocumentRegister(){let rows=getVisibleDocuments().map(f=>`<tr><td>${esc(f.doc_number||"")}</td><td>${esc(f.title||"")}</td><td>${esc(f.status||"Draft")}</td><td>${new Date(f.created_at).toLocaleString()}</td><td>${esc(f.created_by||"")}</td></tr>`).join("");printHTML("Document Register",`<h1>Document Register</h1><table class="official-table"><tr><th>Document #</th><th>Title</th><th>Status</th><th>Created</th><th>Created By</th></tr>${rows}</table>`)}
function cb(v){return v?"☑":"☐"}function val(d,k){return d?.[k]||""}function sigImg(d,k){return d?.[k]?`<img class="print-signature-img" src="${d[k]}">`:""}function ynp(d,k){return{y:d[k]==="Yes"?"☑":"☐",n:d[k]==="No"?"☑":"☐"}}
function buildPJOPrintHTML(f){let d=f.data||{},row=(n,text,k,yes=true)=>{let y=ynp(d,"pjo_"+k);return`<tr><td style="height:43px">${n}. ${text}<br>${esc(d["pjo_"+k+"_comment"]||"")}</td><td style="text-align:center;width:70px">${yes?y.y+"<br>Тийм / Yes":""}</td><td style="text-align:center;width:70px">${yes?y.n+"<br>Үгүй / No":""}</td></tr>`};return`<div class="official-page"><table class="official-table"><tr><td style="font-size:24px;font-weight:900;color:#1e3a8a"><span style="color:#991b1b">D</span>AYAN</td><td colspan="3" class="official-title">АЖЛЫН ТӨЛӨВЛӨГӨӨТ АЖИГЛАЛТ<br>PLANNED JOB OBSERVATION</td></tr><tr><td colspan="2"><b>Ажлын байр / Workplace:</b><br>${esc(val(d,"pjo_workplace"))}</td><td colspan="2"><b>Date of observation:</b><br>${esc(val(d,"pjo_date"))}</td></tr><tr><td colspan="4"><b>Reason for observation:</b><br>${cb(d.pjo_reason_procedure)} Job Procedure / Practice Update &nbsp; ${cb(d.pjo_reason_injury)} Recent injury &nbsp; ${cb(d.pjo_reason_training)} Training follow-up &nbsp; ${cb(d.pjo_reason_experienced)} Experienced worker check &nbsp; ${cb(d.pjo_reason_other)} Other</td></tr><tr><td><b>Job/task observed</b><br>${esc(val(d,"pjo_task"))}</td><td colspan="3"><b>Procedure name and #</b><br>${esc(val(d,"pjo_procedure"))}</td></tr><tr class="official-grey"><td colspan="4">WORKER(S) TO BE OBSERVED</td></tr><tr><td>Worker #1<br>${esc(val(d,"pjo_emp1_name"))}</td><td>Occupation<br>${esc(val(d,"pjo_emp1_occupation"))}</td><td>Experience<br>${esc(val(d,"pjo_emp1_experience"))}</td><td>Trained/licensed?<br>${ynp(d,"pjo_trained").y} Yes ${ynp(d,"pjo_trained").n} No</td></tr><tr><td>Worker #2<br>${esc(val(d,"pjo_emp2_name"))}</td><td>Occupation<br>${esc(val(d,"pjo_emp2_occupation"))}</td><td>Experience<br>${esc(val(d,"pjo_emp2_experience"))}</td><td>Training records verified?<br>${ynp(d,"pjo_training_records").y} Yes ${ynp(d,"pjo_training_records").n} No</td></tr><tr class="official-grey"><td colspan="4">OBSERVATIONS / АЖИГЛАЛТУУД</td></tr></table><table class="official-table">${row(1,"Could any practices or conditions observed result in personal injury or near miss?","q1")}${row(2,"Could any practices or conditions observed result in property damage?","q2")}${row(3,"Were procedures/standards followed?","q3")}${row(4,"What key points were noticed?","q4",false)}${row(5,"Was positive recognition given?","q5")}${row(6,"Was corrective information or instruction given?","q6")}${row(7,"Review for efficiency and production capability?","q7")}</table><div style="text-align:center;font-weight:900;margin-top:10px">SAFETY - FIRST, LAST, AND ALWAYS</div><div style="text-align:right;font-weight:900">Page 1 of 2</div></div><div class="official-page"><table class="official-table">${row(8,"What should we consider changing in the interest of safety?","q8")}${row(9,"Additional comments/observations","q9",false)}${row(10,"Do procedures need to be revised?","q10")}<tr><td>Duration<br>${esc(val(d,"pjo_duration"))}</td><td>Start<br>${esc(val(d,"pjo_start_time"))}</td><td colspan="2">Stop<br>${esc(val(d,"pjo_stop_time"))}</td></tr><tr><td>Recommended follow up<br>${esc(val(d,"pjo_followup"))}</td><td>Responsible person<br>${esc(val(d,"pjo_responsible"))}</td><td colspan="2">Due date<br>${esc(val(d,"pjo_due_date"))}</td></tr><tr class="official-grey"><td colspan="4">PLEASE CONSIDER THE FOLLOWING WHILE OBSERVING</td></tr><tr><td>People: 5 Point Safety System, Procedures, PPE, Ergonomics, Previous Incidents, Training</td><td>Material: Handling, Procedures, Safe Storage, WHMIS</td><td>Equipment/Tools: Pre-use, Maintenance, Guarding, Electrical Hazards, Lock Out/Tag Out</td><td>Environment: Fire, Emergency Exits, Dust/Gas/Noise, Housekeeping, Lighting, First Aid, Eyewash</td></tr></table><table class="official-table" style="margin-top:18px;border:0"><tr><td style="border:0;text-align:center"><div class="print-line">${esc(val(d,"pjo_emp1_name"))}</div>Worker #1 Name</td><td style="border:0;text-align:center"><div class="print-line">${sigImg(d,"pjo_sig_emp1")}</div>Worker #1 Signature</td></tr><tr><td style="border:0;text-align:center"><div class="print-line">${esc(val(d,"pjo_emp2_name"))}</div>Worker #2 Name</td><td style="border:0;text-align:center"><div class="print-line">${sigImg(d,"pjo_sig_emp2")}</div>Worker #2 Signature</td></tr><tr><td style="border:0;text-align:center"><div class="print-line">${esc(val(d,"pjo_observer"))}</div>Observer / Crew Trainer Name</td><td style="border:0;text-align:center"><div class="print-line">${sigImg(d,"pjo_sig_observer")}</div>Observer Signature</td></tr><tr><td style="border:0;text-align:center"><div class="print-line">${esc(val(d,"pjo_supervisor"))}</div>Supervisor Name</td><td style="border:0;text-align:center"><div class="print-line">${sigImg(d,"pjo_sig_supervisor")}</div>Supervisor Signature</td></tr><tr><td style="border:0;text-align:center"><div class="print-line"></div>Superintendent / Area Manager Name</td><td style="border:0;text-align:center"><div class="print-line">${sigImg(d,"pjo_sig_manager")}</div>Manager Signature</td></tr><tr><td style="border:0;text-align:center"><div class="print-line"></div>Safety Coordinator Name</td><td style="border:0;text-align:center"><div class="print-line">${sigImg(d,"pjo_sig_safety")}</div>Safety Coordinator Signature</td></tr></table><div style="text-align:right;font-weight:900">Page 2 of 2</div></div>`}
function buildSCCPrintHTML(f){let d=f.data||{};return`<div class="official-page"><table class="official-table"><tr><td style="border:0;font-size:24px;font-weight:900;color:#1e3a8a"><span style="color:#991b1b">D</span>AYAN</td><td colspan="3" style="border:0;text-align:center;font-size:18px;font-weight:900">АЮУЛГҮЙ АЖИЛЛАГААНЫ ШАТАЛСАН КӨҮЧИНГ/<br>SAFETY CASCADING COACHING</td></tr><tr class="official-light"><td>Date<br>${esc(val(d,"scc_date"))}</td><td>Name</td><td>Signature</td><td>Job title<br>${esc(val(d,"scc_job_title"))}</td></tr><tr><td>Name of Learner</td><td>${esc(val(d,"scc_learner"))}</td><td>${sigImg(d,"scc_sig_learner")}</td><td></td></tr><tr><td>Name of Coach #1</td><td>${esc(val(d,"scc_coach1"))}</td><td>${sigImg(d,"scc_sig_coach1")}</td><td></td></tr><tr><td>Name of Coach #2</td><td>${esc(val(d,"scc_coach2"))}</td><td>${sigImg(d,"scc_sig_coach2")}</td><td></td></tr><tr><td>Department / Team</td><td colspan="3">${esc(val(d,"scc_department"))} ${esc(val(d,"scc_team"))}</td></tr><tr class="official-grey"><td colspan="4">Which safety process was conducted during the coaching session?</td></tr><tr><td>${cb(d.scc_process_preshift)} Pre-Shift Meeting</td><td>${cb(d.scc_process_discussion)} Safety Discussion</td><td>${cb(d.scc_process_crm)} CRM</td><td>${cb(d.scc_process_ptha)} Pre-Task Hazard Assessment<br>${cb(d.scc_process_other)} Other</td></tr><tr class="official-grey"><td colspan="4">Supervisor, what went well?</td></tr><tr><td colspan="4" style="height:130px">${esc(val(d,"scc_self_went_well"))}</td></tr><tr class="official-grey"><td colspan="4">Supervisor Self-analysis: suggested improvements</td></tr><tr><td colspan="4" style="height:130px">${esc(val(d,"scc_self_improvements"))}</td></tr></table><div style="text-align:right;font-weight:900">Page 1 of 2</div></div><div class="official-page"><table class="official-table"><tr><td style="border:0;font-size:24px;font-weight:900;color:#1e3a8a"><span style="color:#991b1b">D</span>AYAN</td><td colspan="3" style="border:0;text-align:center;font-size:18px;font-weight:900">АЮУЛГҮЙ АЖИЛЛАГААНЫ ШАТАЛСАН КӨҮЧИНГ/<br>SAFETY CASCADING COACHING</td></tr><tr class="official-grey"><td colspan="4">1 COACHING DISCUSSION:</td></tr><tr><td colspan="4" style="height:160px">${esc(val(d,"scc_discussion1"))}</td></tr><tr><td colspan="4"><b>Coach suggested improvements:</b><br>${esc(val(d,"scc_discussion1_improve"))}</td></tr><tr class="official-grey"><td colspan="4">2 COACHING DISCUSSION:</td></tr><tr><td colspan="4" style="height:150px">${esc(val(d,"scc_discussion2"))}</td></tr><tr class="official-grey"><td colspan="4">SUMMARIZE COACHING SESSION:</td></tr><tr><td colspan="4" style="height:150px">${esc(val(d,"scc_summary"))}<br><br><b>Key actions:</b><br>${esc(val(d,"scc_actions"))}</td></tr></table><div style="text-align:right;font-weight:900">Page 2 of 2</div></div>`}
function printHTML(title,content){let w=window.open("","_blank");if(!w)return alert("Popup blocked.");w.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${esc(title)}</title><style>body{font-family:Arial,sans-serif;margin:0;color:#000}.official-page{box-sizing:border-box;width:100%;page-break-after:always}.official-page:last-child{page-break-after:auto}.official-table{width:100%;border-collapse:collapse;font-size:10.2px;line-height:1.12}.official-table td,.official-table th{border:1px solid #111;padding:3.5px;vertical-align:top}.official-title{text-align:center;font-weight:900;font-size:16px}.official-grey{background:#707070!important;color:white!important;font-weight:900;-webkit-print-color-adjust:exact;print-color-adjust:exact}.official-light{background:#e5e5e5!important}.print-signature-img{max-width:185px;max-height:52px}.print-line{border-bottom:1px solid #111;height:36px;text-align:center}@media print{@page{size:A4 portrait;margin:7mm}body{margin:0}}</style></head><body>${content}</body></html>`);w.document.close();w.focus();setTimeout(()=>w.print(),500)}
function printCurrentForm(){let t=document.getElementById("formType")?.value||"pjo",title=document.getElementById("formType")?.selectedOptions[0]?.textContent||"Form",f={type:t,title,data:collectCurrentForm()};printHTML(title,t==="cascaded"?buildSCCPrintHTML(f):buildPJOPrintHTML(f))}
function printSingleSavedForm(id){let f=savedForms.find(x=>x.id===id);if(!f)return alert("Document not found.");printHTML(f.doc_number||f.title,f.type==="cascaded"?buildSCCPrintHTML(f):buildPJOPrintHTML(f))}
function roleAction(action){if(action==="documents"){showTab("documents");renderDocumentCentre();return}if(action==="forms"){showTab("forms");return}if(action==="entries"||action==="dashboard"){showTab("entriesRegister");if(typeof renderEntriesRegister==="function")renderEntriesRegister();return}if(action==="entry")showTab("entry");else if(action==="gallery")showTab("gallery");else if(action==="map")showTab("map");else if(action==="ai")showTab("ai");else if(action==="report")showTab("report");else if(action==="actions")showTab("actions");else if(action==="admin")showTab("admin")}
function applyRoleView(){let role=getRoleName?getRoleName():"Supervisor",title=document.getElementById("roleDashboardTitle"),sub=document.getElementById("roleDashboardSubtitle"),panel=document.getElementById("roleViewPanel"),scope=document.getElementById("roleScopePanel");if(!title||!panel)return;let cfg={Supervisor:["Supervisor Dashboard","Create entries and official documents.",[["Development Entry","entry"],["Official Forms","forms"],["My Documents","documents"],["My Entries","entries"]],[["Own Entries","entries"],["Own Documents","documents"],["Own Photos","gallery"],["Own Actions","actions"]]],Superintendent:["Superintendent Dashboard","Review area entries, documents and actions.",[["Area Entries","entries"],["Document Centre","documents"],["Forms Review","documents"],["Open Actions","actions"]],[["Area Entries","entries"],["Area Documents","documents"],["Area Photos","gallery"],["KPI / Delays","entries"]]],Manager:["Manager Dashboard","Department-wide production, documents and reports.",[["All Entries","entries"],["Document Centre","documents"],["All Forms","documents"],["AI Executive Summary","ai"]],[["All Entries","entries"],["All Documents","documents"],["All Photos","gallery"],["Executive Trends","ai"]]],Admin:["Admin Dashboard","System administration and all records.",[["User Management","admin"],["Document Centre","documents"],["All Entries","entries"],["All Forms","documents"]],[["Users / Roles","admin"],["Database Health","admin"],["All Records","documents"],["Developer View-As","dashboard"]]]}[role]||[];title.textContent=cfg[0];sub.textContent=cfg[1];panel.innerHTML=`<div class="role-banner"><h3>${esc(role)} View${previewRole?" (Preview)":""}</h3><p>${esc(cfg[1])}</p><div class="role-list">${cfg[2].map(a=>`<button class="role-action-btn" onclick="roleAction('${a[1]}')">${esc(a[0])}</button>`).join("")}</div></div>`;if(scope)scope.innerHTML=`<div class="scope-grid">${cfg[3].map(s=>`<button class="scope-card-btn" onclick="roleAction('${s[1]}')"><b>${esc(s[0])}</b><span>${esc(role)} access</span></button>`).join("")}</div>`}
setTimeout(()=>{initSignaturePad();applyLanguage();},700);
