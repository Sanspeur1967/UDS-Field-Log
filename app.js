...
/* Enterprise 5.0 - Smart Forms Engine */
let smartSaveTimer=null;
function smartEnsureStatus(){const forms=document.getElementById('forms');if(!forms||document.getElementById('smartFormStatus'))return;const box=document.createElement('div');box.id='smartFormStatus';box.className='smart-form-status';box.textContent='🟢 Ready — changes save locally when you save the form';const title=forms.querySelector('.section-title');if(title)title.insertAdjacentElement('afterend',box)}
function smartSetStatus(t){smartEnsureStatus();const b=document.getElementById('smartFormStatus');if(b)b.textContent=t}
function smartResizeTextarea(el){if(!el)return;el.style.height='auto';el.style.height=Math.max(92,el.scrollHeight+4)+'px'}
function smartTextAreaEvents(){document.querySelectorAll('#currentFormPrintable textarea').forEach(t=>{t.classList.add('smart-textarea');smartResizeTextarea(t);if(t.dataset.smartReady)return;t.dataset.smartReady='1';t.addEventListener('input',()=>{smartResizeTextarea(t);smartSetStatus('🟡 Editing — remember to Save before leaving');clearTimeout(smartSaveTimer);smartSaveTimer=setTimeout(()=>smartSetStatus('🟢 Ready — local draft is safe on this screen'),900)});t.addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){const v=t.value,start=t.selectionStart,before=v.slice(0,start),lineStart=before.lastIndexOf('\n')+1,current=before.slice(lineStart).trim();if(current.startsWith('•')){e.preventDefault();const ins='\n• ';t.value=v.slice(0,start)+ins+v.slice(t.selectionEnd);t.selectionStart=t.selectionEnd=start+ins.length;smartResizeTextarea(t)}}})})}
const smartOldRenderSelectedForm=typeof renderSelectedForm==='function'?renderSelectedForm:null;function renderSelectedForm(){if(smartOldRenderSelectedForm)smartOldRenderSelectedForm();setTimeout(()=>{smartEnsureStatus();smartTextAreaEvents()},50)}
function smartSafe(x){return typeof esc==='function'?esc(x||''):String(x||'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}
function smartBullets(text){text=String(text||'').trim();if(!text)return '';let lines=text.split(/\n+/).map(s=>s.trim()).filter(Boolean).map(s=>s.replace(/^[-•*\u2022]\s*/,'').trim()).filter(Boolean);if(lines.length===1&&lines[0].includes(';'))lines=lines[0].split(';').map(s=>s.trim()).filter(Boolean);if(lines.length<=1)return `<div class="single-answer">${smartSafe(lines[0]||text)}</div>`;return `<ul>${lines.map(s=>`<li>${smartSafe(s)}</li>`).join('')}</ul>`}
function smartContinuationPage(title,sections){const filled=sections.filter(s=>String(s.text||'').trim().length>450);if(!filled.length)return '';return `<div class="official-page smart-continuation"><table class="official-table"><tr><td style="border:0;width:22%" class="official-logo"><span>D</span>AYAN</td><td colspan="3" style="border:0" class="official-title">CONTINUATION PAGE<br>НЭМЭЛТ ХУУДАС</td></tr><tr class="official-grey"><td colspan="4">${smartSafe(title)}</td></tr>${filled.map(s=>`<tr class="smart-section"><td colspan="4"><b>${smartSafe(s.label)}</b><div class="smart-answer">${smartBullets(s.text)}</div></td></tr>`).join('')}</table><div class="print-footer"><span>Continuation Page</span><span>Generated by UDS Development Pro</span></div></div>`}
/* Add continuation to approved Cascaded print without changing main layout */
const smartOldBuildSCC=typeof buildSCCPrintHTML==='function'?buildSCCPrintHTML:null;function buildSCCPrintHTML(f){const base=smartOldBuildSCC?smartOldBuildSCC(f):'';const d=(f&&f.data)||{};const cont=smartContinuationPage('Safety Cascading Coaching',[{label:'Supervisor, what went well?',text:d.scc_self_went_well},{label:'Supervisor Self-analysis',text:d.scc_self_improvements},{label:'1 Coaching Discussion',text:d.scc_discussion1},{label:'Coach suggested improvements',text:d.scc_discussion1_improve},{label:'2 Coaching Discussion',text:d.scc_discussion2},{label:'Summary and Key Actions',text:(d.scc_summary||'')+'\n'+(d.scc_actions||'')}]);return base.replace('</div>',cont+'</div>')}
const smartOldPrintHTML=typeof printHTML==='function'?printHTML:null;function printHTML(title,content){const win=window.open('','_blank');if(!win)return alert('Popup blocked. Allow popups for this site.');win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>${smartSafe(title)}</title><style>body{font-family:Arial,"Noto Sans",sans-serif;margin:0;color:#000;background:white}.official-page{box-sizing:border-box;width:100%;min-height:calc(297mm - 14mm);page-break-after:always}.official-page:last-child{page-break-after:auto}.official-table{width:100%;border-collapse:collapse;table-layout:fixed;font-size:9.6px;line-height:1.12;color:#000}.official-table td,.official-table th{border:1px solid #111;padding:3px;vertical-align:top;word-wrap:break-word}.official-title{text-align:center;font-weight:900;font-size:16px}.official-logo{font-size:24px;font-weight:900;color:#1e3a8a}.official-logo span{color:#991b1b}.official-grey{background:#6f6f6f!important;color:#fff!important;font-weight:900;-webkit-print-color-adjust:exact;print-color-adjust:exact}.official-light{background:#e5e5e5!important;-webkit-print-color-adjust:exact;print-color-adjust:exact}.print-signature-img{max-width:180px;max-height:45px}.print-footer{display:flex;justify-content:space-between;align-items:center;margin-top:6px;font-size:10px;font-weight:900}.answer-cell,.smart-answer{font-size:12px;font-weight:700;line-height:1.42;vertical-align:top!important;padding:10px 14px!important;white-space:normal}.answer-cell ul,.smart-answer ul{margin:0;padding-left:24px}.answer-cell li,.smart-answer li{margin:4px 0;break-inside:avoid}.single-answer{text-align:left}.scc-header-answer{font-size:12px;font-weight:700;text-align:center;vertical-align:middle!important}.smart-section{break-inside:avoid;page-break-inside:avoid}.smart-continuation{page-break-before:always}@media print{@page{size:A4 portrait;margin:7mm}body{margin:0!important;background:white!important}}</style></head><body>${content}</body></html>`);win.document.close();win.focus();setTimeout(()=>win.print(),500)}
setTimeout(()=>{smartEnsureStatus();smartTextAreaEvents()},800);
