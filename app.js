...
/* 5.8.1 PJO Signature Hotfix */
(function(){
function esc581(x){return String(x||'').replace(/[&<>"']/g,function(m){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m];});}
function card(title,nameId,sigId){
  var sig='';
  if(typeof window.sigField==='function') sig=window.sigField(title,sigId);
  else sig='<div class="signature-card"><div>No signature captured</div><button type="button">Sign</button><button type="button">Clear</button></div>';
  return '<div class="pjo-signature-card"><div class="pjo-signature-title">'+esc581(title)+'</div><label>Typed Name / Нэр<input id="'+nameId+'" placeholder="Type name here"></label>'+sig+'</div>';
}
function isPJO(form){if(!form)return false; var t=(form.textContent||'').toLowerCase(); return t.indexOf('planned job observation')>=0 || t.indexOf('ажлын')>=0 || !!form.querySelector('[id^="pjo_"]');}
window.e581AddPJOSignatures=function(){
  var form=document.querySelector('#currentFormPrintable'); if(!isPJO(form)) return false;
  document.querySelectorAll('#pjoSignatureSection').forEach(function(x){x.remove();});
  var section=document.createElement('div'); section.className='form-section'; section.id='pjoSignatureSection';
  section.innerHTML='<h3>Signatures / Гарын үсэг</h3><p>Type the name first, then tap <b>Sign</b>. Apple Pencil or finger can be used.</p><div class="pjo-signature-grid">'+
  card('Worker #1 Signature','pjo_name_emp1','pjo_sig_emp1')+card('Worker #2 Signature','pjo_name_emp2','pjo_sig_emp2')+card('Observer / Crew Trainer Signature','pjo_name_observer','pjo_sig_observer')+card('Supervisor Signature','pjo_name_supervisor','pjo_sig_supervisor')+card('Superintendent / Area Manager Signature','pjo_name_manager','pjo_sig_manager')+card('Safety Coordinator Signature','pjo_name_safety','pjo_sig_safety')+'</div>';
  var btn=form.querySelector('.save-btn,button[onclick*=saveFillableForm]'); if(btn&&btn.parentElement) btn.parentElement.insertAdjacentElement('beforebegin',section); else form.appendChild(section); return true;
};
function inject(){setTimeout(window.e581AddPJOSignatures,100);setTimeout(window.e581AddPJOSignatures,400);setTimeout(window.e581AddPJOSignatures,900);}
['e56OpenForm','renderSelectedForm','newForm'].forEach(function(fn){var old=window[fn]; if(typeof old==='function') window[fn]=function(){var r=old.apply(this,arguments); inject(); return r;};});
